...
def extract_feature_map(train, resolution, perplexity):
    transformer = DeepInsightTransformer(pixels=resolution, perplexity=perplexity)
...
class DeepInsightTransformer:
    ...
    def __init__(
        ...
        if isinstance(feature_extractor, str):
            ...
            if fe == "tsne_exact".casefold():
                ...
                raise ValueError(
                    ("Feature extraction method '{}' not accepted").format(
                        feature_extractor
                    )
                )
            self._fe = fe
            ...
            raise TypeError(
                "Parameter feature_extractor is not a "
    ...
    def fit(self, X, y=None, plot=False):
        ...
        if plot is True:
            ...
            ax[0, 0].fill(
                x_new[chvertices, 0], x_new[chvertices, 1], edgecolor="r", fill=False
            )
    ...
    def _calculate_coords(self):
        ...
        ax0_coord = (
            np.digitize(
                self._xrot[:, 0],
                bins=np.linspace(
                    min(self._xrot[:, 0]), max(self._xrot[:, 0]), self._pixels[0]
                ),
            )
            - 1
        )
        ax1_coord = (
            np.digitize(
                self._xrot[:, 1],
                bins=np.linspace(
                    min(self._xrot[:, 1]), max(self._xrot[:, 1]), self._pixels[1]
                ),
            )
            - 1
        )
    ...
    def transform(self, X, empty_value=0):
        ...
        img_coords = (
            pd.DataFrame(np.vstack((self._coords, X.clip(0, 1))).T)
            .groupby([0, 1], as_index=False)  # (x1, y1)
            .mean()
        )
        ...
        for z in range(2, img_coords.shape[1]):
            ...
            img_matrix[
                img_coords[0].astype(int), img_coords[1].astype(int)
            ] = img_coords[z]
    ...
    def feature_density_matrix(self):
        ...
        # Tranpose to get (n_features, n_samples)
        coord_cnt = (
            pd.DataFrame(self._coords.T)
            .assign(count=1)
            .groupby([0, 1], as_index=False)  # (x1, y1)
            .count()
        )
        fdmat[coord_cnt[0].astype(int), coord_cnt[1].astype(int)] = coord_cnt["count"]
        return fdmat
